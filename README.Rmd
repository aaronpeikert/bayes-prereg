...
Much has been theorized on this vital distinction between exploration and confirmation, discovery and justification, hypothesis-generating and hypothesis-testing and so forth (see XXXX).
...
Rigorous application of preregistration prevents researchers from reporting a simple confirmatory story for a set of results produced by an arduous process of try and error (XXXX).
...
To uphold the simplicity of the decision rule, it is routinely suggested only to preregister when a study is expected to have a low type I error rate by design and will run/be analyzed without changes no matter what (XXX).
...
To that end, we first introduce some tools of Bayesian philosophy of science and map the exploration/confirmation distinction onto a dimensional quantity we call "theoretical risk" (a term borrowed from Meehl), which is inversely related to type I error rate.
...
This argument is not self-evident for a conformationist, so we make a lengthy argument for why an inductionist should care.
We assume that a falsificationist is generally interested in the probability of finding evidence against a theory if it is wrong.
...
When we put these arguments into the Bayes' formula, we conclude that because preregistration decrease the uncertainty about the theoretical risk, which in turn increases our expectation to gain evidence for or against a theory, preregistration are warranted for any study.
...
This conceptualization of choosing a study as a betting problem lets us apply a "Dutch Book" argument (XXX).
...
Our first argument is that risky predictions create persuasive evidence if they turn out to be correct.
...
This is not a risky prediction to make in psychology, even without statistical issues like alpha inflation (cite crud factor XXX).
...
At last, we can not help but speculate about a sociological factor at play.
If preregistrations aim to increase theoretical risk, then even researchers who are sold on the merits may deem the associated "high risk; high reward"-strategy as unfit for the incentive structure they operate under.

# Uncertainty about Theoretical Risk 

We established that higher theoretical risk leads to more credible evidence.
Nevertheless, there are trait offs for increasing it.
...
For those models or assumptions where this is impossible, one can employ Monte Carlo simulation to estimate the contribution $P(E|¬H)$.
...
We have to remember that the process of weighing evidence for or against a theory extends to the original authors and all the people they hope to pursue.
...
Before we ask how preregistration is influencing uncertainty about theoretical risk, we must consider the implications of being uncertain.
...
We chose this uniform distribution to capture our statement that a statistical model only guarantees a lower bound to $p(E|\neg{}H)$ (and since its a probability the upper bound is 1) as it is the maximum entropy distribution under this assumption and conforms therefore with our Bayesian framework (https://arxiv.org/pdf/0708.1593.pdf XXX).
...
Generally, we expect that uncertainty (and hence more entropic distributions) lead to a decreased expected epistemic value.
