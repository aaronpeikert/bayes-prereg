...
abstract: |
  ...
  Preregistration effectively reduces this uncertainty and therefore generates more persuasiveness evidence.
---
...
<!--this does not belong to the introduction-->
...
The first is that theoretical risk is important for judging theories.
...
When we put these arguments into the Bayes' formula, we conclude that decreasing uncertainty about the theoretical risk increases our expectation to gain evidence for or against a theory.
We will later discuss how our argument relates to a frequentist philosophy of science.
For now we stick to a Bayesian rational since it is straightforward to incorporate uncertainty; a move that would require considerable thought otherwise.
...
In carnaps tradition, in its direct use, it is called corroboration as firmness; in its relation to $P(H)$, it is called increase in firmness.[^corroboration]
<!--I think I get myself in hot water here, one the one hand I claim compatibility of hypo-deductive methods and the other hand I appeal to carnap!!!-->
...
[^corroboration]: This move to assume measures of corroboration is a leap of faith and not covered by the Dutch book argument we make earlier, so a falsificationist can rightfully reject it.
...
We argue that while such conclusion it plausible, we must first consider at least two constrains that put an upper bound to the theoretical risk.

First, the theory itself.
...
But our imaginary researcher considers the relation of these two sets and for lack of a better theory they assume that some relation between any variables of the two sets exists.
This is not a risky prediction to make in psychology even without statistical issues like alpha inflation (cite crud factor).
However, we would consider it an success if the researcher would use the evidence to develop a more precise (and hence risky) theory, e.g. by proposing which variables from the one set relate to which from the other set to what extend, direction, functional shape etc.
...
Increasing $P(¬E|¬H)$ will usually decease $P(E|H)$ unless more resources are invested.
...
As we alluded to earlier the resource and incentive structure of research also does not make this easier.

# Uncertainty about Theorethical Risk 
...
In other words we have reconstructed the interpretation that preregistrations restrict the researchers which increases theoretical risk (or equivalently limit type I error rate) and thereby creates more impactfull evidence.
However, employing a mathematical framework allows us to navigate the trade offs more effectively and move towards a second, more favorable, interpretation.
To that end, we incorporate uncertainty into the our mathematical framework.
...
These are all tools that help us factor in the tendency of statistical methods to show results even when there are none and are therefore closely related to theorethical risk.
...
While in theory these factors may leave $P(E|¬H)$ unaffected or even decrease it, we argue that in practice this is not the case.
...
Put simply, the only thing we can conclude with some certainty is that $P(E|¬H)$ is not lower than what the statistical model guarantees.

## The effects of uncertainty

Before we ask how preregistration is influencing uncertainty about theorethical risk, we must consider 

# Preregistration as a Means to Decreasing Uncertainty about the Theoretical Risk

We hope to have persuaded you to accept two arguments: First, theoretical risk is important for judging evidential support for theories.
Second, the theorethical risk is inherently uncertain, which diminishes the persuasiveness of the gathered evidence.
