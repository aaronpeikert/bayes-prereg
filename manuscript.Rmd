...
However, it is generally expected that any measure of corroboration (and even increases monotonically with an increase in posterior probability ($P(H|E)$).
...
To make these concepts less abstract, consider the following illustrative example of a theory and different experiments that test it.
Suppose the theory is about the cognitive phenomenon of "insight", and allow us to define it somewhat hand-waving as an cognitive abstraction that allows agents to consistently solve a well defined class of problems.
...
All take a sample of ten psychology students and present them the problem for brief span of time.
After that:

1. the experimenter explains that with roman-numerals the problem is easy to solve; if all students come up with the solution, she records it as evidence for the hypothesis.
...
The experiment 2 on the other hand, has low theoretical risk ($P(¬E_2|¬H)$).
Even if "insight" has nothing to do with solving the problem ($¬H$), there are other plausible reasons for observing the evidence ($E_2$), because the students can simply copy the solution, without any insight.
With regard to detectability experiment 1 and 2 differ in no obvious way.
Experiment 3, however, has low detectability.
...
Of course, it comes to mind to allow the evidence to be probabilistic, e.g., relax the requirement of "all students" to nine out of ten students, more than eight, and so forth.
...
Preregistration is often discussed in relation to statistical methods, so let us note some relations between $P(E|¬H)$ (the inverse of theorethical risk) and statistical methods.
...
We argue that $P(E|¬H)$ (and hence theorethical risk) also encompasses factors outside the statistical realm, most notably the study design and broader analytical strategies.
