...
<!--AB: i feel that the intro needs a slower start and take the reader by the hand -->
Imagine three researchers named Ben, Holly and the Wise Old Elf. <!--AB you may want to change these names--> 
Based on his theoretical considerations, Ben predicts that personality traits are useful predictors of job success. 
Based on her own theoretical considerations Holly specifically predicts that agreeableness is a good predictor of job success and she specifically predicts a correlation of 0.5. The Wise Old Elf who did not have a theory to begin with waits until Ben and Holly collected their data and after examining all correlations of job success and personality traits postdicts that indeed agreeableness is correlated with job success (which he claims to have known all along).
Which theory should we attribute the largest credibility?

Voice from off:  could we discuss precision of a theory and the exploration/confirmation distinction with an example like this?? And probably a lot more... let's discuss!
-->
...
Under these conditions, the rule upholds, but such restricted use makes preregistration a niche solution unable to match the greater problem of replicability in psychological research or elsewhere.
...
Researchers cannot know what they will learn from a study before they have run it. However, they can develop an expectation that helps them to decide which study to conduct <!--AB in the sense of maximum information gain? What does "which study" mean? I guess you mean just any possible property of an study in the planning phase that can be altered?-->.
...
Our first argument is that theorethical risk is crucial for judging theories.<!-- again: judging what?-->
...
We restrict ourselves to evidence of a binary nature (either exists or does not exist) <!--this seems like an ontological level but here again, aren't we on an epistemic level, so shouldn't it be: was observed or not? --> since continuous evidence would lead to some quite involved derivations.
<!--AB: What about measurement error? Is evidence thought to be measurement-free her? -->
We denote the probability <!--AB: personally, I think that a lot of confusion with Bayesian results is that the 'term' probability makes it sound objective and truthful; would you be okay with replacing probability with credibility to underline the subjective nature of this approach? Or would this be even more confusing?--> of a hypothesis before observing evidence as $P(H)$ and its complement as $P(¬H) = 1 - P(H)$.
<!--AB: Not all readers may understand what the probability of a hypothesis is. Elaborate more on the probability interpretation of Bayesians as credibility.-->
...
In the tradition of Carnap (XXX), in its direct use, it is called corroboration as firmness; in its relation to the prior believe in the hypothesis, $P(H)$, it is called increase in firmness.
...
In short, to support our theory as good as possible, we want to increase $P(H|E)$. <!-- or: to corroborate or to confirm or...?-->
Increases in $P(H|E)$ are associated with increased epistemic value, of which we want to maximize the expectation. <!--Why the expectation? Because you take the expectation of an entire field or a scientist who does multiple experiments over her/his career? -->
...
The first option leads us to the unsurprising conclusion that a higher prior believe in a theory, $P(H)$, leads to higher $P(H|E)$.
If a hypothesis is more credible to begin with, the same increase in corroboration will lead to a higher credibility than if the study was less credible to begin with. <!--AB: not sure - state the obvious explicitly?-->
Anyhow, the a priori probability of a hypothesis is nothing our study design can change.
The second option is similar unsurprising; that is, an increase in $P(E|H)$ leads to higher $P(H|E)$.
In other words, researchers should ensure that their study design allows them to find evidence for their hypothesis, in case it is true.
...
<!--AB: from the perspective of a rational agent/researcher, of course, it makes sense to make P(E|H) large but isn't psychology facing a widespread problem of underpowered studies? I would make this very clear that this essentially relates to power (right?) and that historically we haven't been doing well on that account but that this problem is not pertinent to preregistration as such-->
...
<!--AB can we add an example that explains this whole idea once more but with a concrete example? I think that the probability of evidence given NOT H is just difficult to wrap one's head around. It would be ideal to illustrate this with an example that presents both a weak and a strong P(E given not H).

I have a hard time coming up with an easy example that relates this simplified case
to a problem of psychological research.

H: a depression treatment is effective
not H: the treatment is not effective

In a weak risk case, the prediction is fuzzy, P(E | not H) is large, theoretical risk is small:

E: people have some depression score (other than zero) after the treatment
not E: people have exactly zero depression scores

In a strong risk case, the prediction is precise, P(E | not H) is small, theoretical risk is large
E: people improve at least 10 points on the depression scale
not E: people improve less or equal to 10 points on the depression scale
-->


Let us note some interesting properties of theoretical risk, $P(¬E|¬H)$.
<!--AB: I rephrased this sentence. Is credibility equal to epistemic value?-->
First, if the objective is to increase the credibility of a theory $P(H|E)$, this can
be achieved by increasing theoretical risk.
Second, if the theoretical risk is smaller than $P(E|H)$ it follows that $P(H|E)$ must decrease when observing the evidence.
<!--AB: explain more what this means. When is this condition true in reality?-->
...
<!--Is this the word "this" refering to the third point only or all three?-->
...
Indeed, limiting the false positive (or, type I error rate) is commonly stated as a goal of preregistration. <!--AB: citation needed; maybe verbose quote?-->
...
<!--AB: the kind of more concrete example that you give here is needed much earlier when you define the different conditional probabilities; maybe even in the very beginning, it could be helpful to give examples of risky vs non-risky predictions in psychology from published literature?!-->
...
Suppose each set by itself is well studied, and strong theories tell the researcher how the variables within each of the sets relate to each other.
Now, our imaginary researcher considers the relation between these two sets.
For lack of a better theory, they assume that some relation between any variables of the two sets exists.
This is not a risky prediction to make in psychology, even without statistical issues like alpha inflation due to multiple testing [@orbenCrudReDefined2020].
...
We will later show that preregistration increases the belief one can stake in the further specified theory, though it remains low until substantiated by testing it again. <!--AB: "till" is never used in formal writing-->
The point, however, is that we want to show that preregistration increases the expected epistemic value independent of the theory being tested.

Second, available resources limit theoretical risk.
Increasing $P(¬E|¬H)$ will usually decrease $P(E|H)$ unless more resources are invested. <!-- I recommend giving specific names to each of the conditional probabilities and then refer to those names and have the math notation in parentheses; then, this becomes easier to read and understand, e.g.,

Increasing theoretical risk ($P(¬E|¬H)$) will usually decrease the probability of observing the evidence under the hypothesis ($P(E|H)$). 

Is there a term, we can introduce for $P(E|H)$, like power? 
...
Employing a mathematical framework allows us to navigate the trade-offs more effectively and move towards a second, more favourable interpretation.
...
In other words, the probability that a given significant finding reflects a true effect is larger if researchers lower their type I error rates.
However, the research hypothesis is seldomly equal to the statistical null hypothesis.
We argue that $P(E|¬H)$ (and hence its inverse, theorethical risk) also encompasses factors outside the statistical realm, most notably the study design and broader analytical strategies. <!--AB: can you elaborate more?!-->
...
These tools help us account for biases in statistical methods that increase the likelihood of signifying results even when there are none.
...
## Sources of Uncertainty <!--AB really causes?-->
...
However, other factors in the broader context of a given study are much harder to quantify.
...
Under ideal circumstances (assumption of independence, normality of residuals, equal variance), it will keep its promised type I error rate.
...
They might exclude outliers, choose to drop an item before computing a sum score, broaden their definition of the population to be sampled (thus jeopardizing distributional assumptions<!-- AB: is this what you meant?-->), translate their questionnaires into a different language without validating it, impute missing values, or any number of other things.
...
While one can hope that processing this information does not affect their decision making either way, this cannot be ruled out.
We, therefore, conclude that statistical properties only guarantee a lower bound to the quantity we seek to minimize.
The only thing we can conclude with some certainty is that $P(E|¬H)$ is not lower than what the statistical model guarantees without knowledge about other factors that play a role.
...
Uncertainty about theoretical risk is expressed through the entropy of the distribution. Under Gaussian assumptions, this is proportional to the variance of the distribution.
...
<!--AB: I am confused that a new section heading is following but I haven't still understood or seen what follows from the expectation that you just gave. I assume the problem is that you created Figures 1 & 2 to illustrate the consequences of the choices you made and the expectation you presented but you do not refer to these Figures from the text, o the reader is lost and may never read the figures... please refer to the figures and explain what is going on in the text. Currently, I do not know what to take from Figure 2 other than that there are different ways to formalize this. Please also cite all sources for these measures in the text.-->
...
Second, the theoretical risk is inherently uncertain and the degree of uncertainty diminishes the persuasiveness of the gathered evidence. <!--AB my rewrite correct?-->
...
Communicating clearly what the authors did to arrive at the evidence (<!--AB: "to arrive at the evidence" is unclear to me, do you mean a conditional probability or really the evidence itself, say, in numeric form in a data table)? --> is crucial for judging the theoretical risk they took.
...
That is not to say that a study that was not preregistered was subjected to practices of p hacking or other questionable research practices.
The point is, as a sceptic, we cannot exclude this and a myriad of other possibilities that may bias the conclusions drawn from the presented evidence and, hence, are left in a state of uncertainty.
If the researchers do describe what they intend to do beforehand, and then report they did exactly that, this uncertainty is drastically reduced.
They still might be uncertain about the concrete theoretical risk the authors took but much less so.
Remaining sources of uncertainty might be unfamiliarity with statistical methods or experimental paradigms used, the probability of an implementation error in the statistical analyses, a bug in the software used for analysies, etc.
...
We started out with the observation that preregistrations do no always cleanly distinguish exploration from confirmation. 
...
As we further argued, minimizing the uncertainty about theoretical risk (without necessarily changing the theoretical risk itself) has benefits for the epistemic value, too, but without increasing the likelihood of producing a "failed" study. <!--AB: is my rephrasing correct?-->
Note that any single study that does not produce evidence in favor of a research hypothesis is still precious for the scientific process. <!--AB: is my rephrasing correct? Please elaborate more. Why is it helpful? In the sense of a meta-analysis? -->
Under the current incentive structure in academia, however, many researchers have reason to avoid such negative outcomes and few, if any, have the privilege to operate outside of these incentives. <!--AB: but see registered reports, journals that increasingly publish null findings, Bayesian analyses etc; I am not sure whether we need these last two sentences? -->

In our view, the advantage of underlining uncertainty reduction about theoretical risk front and center is two-fold.
First, aiming to reduce uncertainty is beneficial across a wide range of potential theoretical risks. <!--AB: do you refer to the risk functions in Fig2? I am not sure; or type of studies?!-->
That is, researchers benefit from preregistrating their study, regardless whether or not the study is "exploratory". <!--AB: this point, I still do not get; the more exploratory a study, the less I can reduce uncertainty to the extreme that I have full uncertainty; but I assume your argument is that whatever reduction in uncertainty is possible is beneficial; but in practice: as a reader: how can I really judge this continuum of uncertainty from a given manuscript??-->
If researchers conduct a more exploratory study they can clearly communicate how exploratory they aim to be and their results can be judged accordingly. <!--AB to repeat: this is still a claim that yet has to stand the test of reality. how should we do this really? I think we need to stress that this is a philosophical perspective rather than a tutorial on how to do this in practice. But... can we give practical advise?-->
...
Allowing to deviate from the preregistration is controversial (XXX) because the authors might sift through all the possibilities and then select the most beneficial for their theory. <!--AB: can you connect this to the probability terms, e.g., thus they reduce their actual theoretical risk through QRPs?-->
