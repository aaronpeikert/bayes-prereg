...
Much has been theorized on this vital distinction between exploration and confirmation, discovery and justification, hypothesis-generating and hypothesis-testing and so forth [see @hoyningen-hueneContextDiscoveryContext2006].
To honour this distinction, many researchers preregister their studies to produce results that are considered confirmatory [@nosekPreregistrationRevolution2018].
Rigorous application of preregistration prevents researchers from reporting a simple confirmatory story for a set of results produced by an arduous process of try and error [@wagenmakersAgendaPurelyConfirmatory2012].
...
Asking researcher to invest time and resources into preregistration demands a principled justification for its utility.

One reason why researchers are hesitant to preregister is that they feel too limited by preregistration [@goldin-meadowWhyPreregistrationMakes2016].
Even researchers who have been persuaded of its advantages sometimes apply strategies to skirt the self-imposed restrictions.
...
This rule of thumb is often assumed implicitly.
...
When we put these arguments into the Bayes' formula, we conclude that because preregistration decrease the uncertainty about the theoretical risk, which in turn increases our expectation to gain evidence for or against a theory, preregistration is warranted for any study.
...
This conceptualization of choosing a study as a betting problem lets us apply a "Dutch Book" argument [@christensenCleverBookiesCoherent1991].
...
In Carnaps (xxx) tradition, in its direct use, it is called corroboration as firmness; in its relation to $P(H)$, it is called increase in firmness.
...
Let us note some interesting properties of theoretical risk $P(¬H|¬E)$.
...
This sounds like a truism but is directly related to Popper's famous criterion of demarcation.
He stated that if it is impossible to prove a hypothesis false, therefore $P(E|¬H) = 1$ or conversely $P(¬E|¬H) = 0$, it can not be a scientific hypothesis [@popperLogicScientificDiscovery2002, p. 18].
...
In fact, limiting type I error rate is commonly stated as a goal of preregistration.
We argue that while such a conclusion is plausible, we must first consider at least two constraints that put an upper bound on the theoretical risk.

First is the theory itself.
Some theories simply do not make risky predictions, and preregistration will not change that.
It would show a great deal of arrogance to advise researchers to stop investigating such theories.
Consider the case of a researcher contemplating the relation between two sets of variables.
Suppose each set considered alone is well studied, and strong theories tell the researcher how the variables within the set relate.
But our imaginary researcher considers the relation between these two sets.
...
However, we would consider it a success if the researcher would use the evidence to develop a more precise (and hence risky) theory, e.g. by proposing which variables from the one set relate to which from the other set to what extent, direction, functional shape etc.
We will later show that preregistration increases the expected epistemic value of the further specified theory, though it remains low till substantiated by testing it again.
...
Changing what theories to test is not on our agenda.

The second upper bound to theoretical risk is resources.
...
This objective also raises the problem of changing the plan after data collection started.
One could argue that allowing deviating from the preregistration would allow researchers the same freedoms as having no preregistration at all.
Hypothethically at least, researchers might try any possible deviation and pick the most favorably for their theory.
Hence, allowing deviation would void any otherwise guaranteed lower limits to the theoretical risk.
It follows that allowing deviations is wholly incompatible with the goal to maximize theoretical risk.

# Uncertainty about Theoretical Risk 

We established that higher theoretical risk leads to more credible evidence.
...
Nevertheless, there are trait offs for increasing theoretical risk.
...
Researchers who choose a smaller type I error rate can hence be surer of their results, if significant.
...
These are all tools that help us factor in the tendency of statistical methods to show results even when there are none and are therefore closely related to theoretical risk.

The point is that the contribution of statistical methods to $P(E|¬H)$ can be formally assessed.
For many statistical models, their $P(E|¬H)$ can be analytically computed under some assumptions.
...
However, researchers might do many very reasonable things in the broader context of the study that effect theoretical risk.
...
If researchers want to or not, except under strict blinding, they continuously observe information about how the study is going, which at best does not effect them in any way.
...
The last argument is that preregistrations reduce this uncertainty.
...
While the authors may have a deep insight into what they did and how it might influence the theoretical risk they took, their readers have much greater uncertainty about these factors.
In particular, they never know what relevant factors the authors, intentionally or not, failed to lay open.
From an ultimate sceptic perspective, they may claim maximum uncertainty.
...
Preregistration are ideal to communicate just that, because any description after the fact is suspect to be incomplete.
The authors could have decided to exclude a number of analytic strategies they tried out etc.
That is not to say any not preregistered study was p hacked.
The point is, we can not exclude this and a myriad of other possibilities and, hence, are uncertain.
If the researcher do describe what they intend to do beforehand, and then report they did exactly that, the readers can be certain, that they have received a complete picture of the situation.
...
However, as the name implies, directly maximizing theoretical risk increases the likelihood that a study fails to deliver results favoring the theory.
...
First, aiming to reduce uncertainty is beneficial across a wide range of theoretical risks.
That is, researchers benefit from preregistrating their study, regardless weather or not the study is "exploratory".
