...
This point is crucial because we attribute much of the appeal of a confirmatory research agenda to this fact.
...
However, it is generally expected that any measure of corroboration increases monotonically with an increase in posterior probability $P(H|E)$ and our argument applies to those measures as well.
We assume that any measure shows gain if, and only if, evidence is observed assuming $P(E|H) > P(E|\neg{}H)$.
This assumption is sometimes called statistical relevance condition [@wesleyc.salmonStatisticalExplanation1970; @fetzerStatisticalExplanations1974].
...
Indeed, limiting the type-I error rate is commonly stated as a goal of preregistration [@nosekPreregistrationRevolution2018; @oberauerPreregistrationForkingPath2019; @rubinDoesPreregistrationImprove2020].
...
However, if a hypothesis in a certain study has low theoretical risk, researchers cannot do much to change that.
...
The other factors (detect ability, prior beliefs, measure of epistemic value) and, therefore, the extend of the benefit very of course with the specifics of the study.
Crucially, even exploratory studies benefit from preregistration, e.g., in this scenario with a $\tau$ of `r some_exploration_tau`, moving from uncertain to certain increases the posterior by a factor of `r round(some_exploration_certain/some_exploration_uncertain, 2)`.
...
To summarize, we showed that higher theoretical risk and lower uncertainty about theoretical risk lead to higher expected epistemic value across a variety of measures.
The former result, that increasing theoretical risk leads to higher expected epistemic value, reconstructs the appeal of confirmatory research.
However, theoretical risk is something researcher have only limited control over, e.g., theories are often vague and illdefined, resources are limited, and increasing theoretical risk usually decreases detectability (a special instance of this trade off is the well known tension between alpha error and power).
...
Researchers must develop theories sufficiently and then have the resources to adequately balance detectability against theoretical risk.
A preregistration may at most help to invest some time into developing more specific, hence riskier, implications of a theory, but enabling confirmatory research is a task that requires action from the whole research community.
Incentive structures must be more willing to evaluate not the outcomes of a study but the rigor with which it was conducted [@caganSanFranciscoDeclaration2013; @schonbrodtResponsibleResearchAssessment2022].
Journal editors could encourage theoretical developments and be willing to accept registered reports [@friedLackTheoryBuilding2020; @friedTheoriesModelsWhat2020; @vanrooijTheoryDevelopmentRequires2020; @vanrooijTheoryTestHow2021].
...
Our later result, on the importance of preregistration for minimizing uncertainty, has two important implications.
...
Second, researchers often have to deviate from the preregistration and make data-dependent decisions outside of the preregistration.
...
Any change would increase the uncertainty about the theoretical risk and may even decrease the theorethical risk.
However, such negative outcomes may be offset by an increase in detectability due to the change.
Consider a preregistration that failed to specify what to do with missing values where the researchers subsequently encounter missings.
In such case, the detectability is zero, since the data cannot be analyzed without a decision about how to handle the missing data.
Any such decision would constitute a deviation from the preregistration.
Furthermore, a reader cannot rule out that the researchers leveraged the decision to decrease theoretical risk, i.e., picking among all options the one that delivers the most beneficial results for the theory.
...
Maybe there is one decision that fits well with the theory or with the study design.
Or, if there is no obvious candidate, the researchers could conduct a multiverse analysis of the available options to deal with missings to show the influence of the decision.

Using reduction in uncertainty as the objective for preregistration does not only explain some existing behavior, it allows to form recommendations to improve the practice of preregistration.
Maybe most important, we now have a measure to differentiate a good preregistration from a bad one.
...
However, specifying procedures that can be expected to not work, will lead to deviation, and uncertainty later.

We have proposed a highly specific, yet practical procedure called preregistration as code (PAC) elsewhere [@citepac].
In a PAC, researchers use computer code instead of a verbal description.
...
Another advantage is that with a PAC, deviations from the preregistration can be isolated and inspected.
...
Confirmatory research would be much less persuasive without preregistration, but in our view preregistration simply communicates the extend of confirmation, i.e., theoretical risk, of a study.
