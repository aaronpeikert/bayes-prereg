...
A successful prediction should lend more credibility to a theory than a "postdiction," everything else being equal. <!-- ME: what is a postdiction? I think this word is to difficult if it is not a technical term -->
...
Confusing exploratory findings with confirmed theories has led to a crisis of confidence in the results of empirical sciences [@ioannidisWhyMostPublished2005], and psychology in particular [@opensciencecollaborationEstimatingReproducibilityPsychological2015].
As a response, more and more researchers preregister their studies [@nosekPreregistrationRevolution2018] to honor this distinction and produce results that are considered confirmatory.

Indeed, rigorous application of preregistration prevents researchers from reporting a set of results produced by an arduous process of trial and error as a simple confirmatory story [@wagenmakersAgendaPurelyConfirmatory2012].
...
Specifically, an intuitive conviction is unable to address pragmatic questions, such as what to include in the preregistration and how detailed this information should be, or about the conditions that warrant deviations.
If the research community remains confused about the purpose of preregistration, it is bound to misapply it and hence decrease the efficiency of the scientific endeavor. <!-- what does efficiency mean in this case? Maybe there is a better word -->
...
Taken at face value, this objective implies that research is confirmatory if, and only if, it is preregistered. <!-- This sounds too much like a strawman to me. I assume it implies more that research can be recognized as confirmatory. -->
However, this postulate is not warranted.
Researchers can conduct confirmatory research without preregistration, though it might be difficult to convince other researchers of the confirmatory nature of their research.
...
Thus, insisting on equating preregistration and confirmation has led to criticism that, all things considered, preregistration is actually harmful, and neither sufficient nor necessary to establish confirmation [@szollosiPreregistrationWorthwhile2020; @phamPreregistrationNeitherSufficient2021]

These issues arise from a fundamental missunderstanding of the objective of preregistration.
Researchers are bound to be misled when they delegate an important scientific judgment to a simple decision rule, that only takes into account the existence of a preregistration. <!-- I dont like "confused" -->
...
Equating confirmatory research with preregistered research is only possible for studies that expect a low type-I error rate and will run/be analyzed without in any case [@simmonsPreregistrationWhyHow2021; @bakkerEnsuringQualitySpecificity2020]. <!-- I dont understand this sentence -->
Under these conditions, the rule holds, but such restricted use makes preregistration a niche solution unable to match the greater problem of lacking replicability in psychology and elsewhere.
...
<!-- you don't rely on all of those, I would not state this here -->
...
However, they can develop an expectation that helps them to decide which study to conduct in what manner.
This expectation is what we term expected epistemic value.
<!-- not compelling, deleted -->
...
To avoid unnessesary technicality, we restrict ourselves to evidence of a binary nature. <!-- I am not shure if this is good. Even in NHS, there is no "binary evidence". In fact, I can't think of any example except for a single coinflip. Perhaps you mean non-randomized statistical tests. -->
...
We do not discuss specific measures of corroboration since it is generally expected that any measure of corroboration increases monotonically with an increase in the posterior probability $P(H|E)$. <!-- citation missing -->

In short, we want to increase the posterior probability $P(H|E)$.
Increases in the posterior probability $P(H|E)$ are associated with increased epistemic value, for which we want to maximize the expectation. <!-- very complicated sentence -->
...
If a hypothesis is more probable to begin with, the same increase in corroboration will lead to a higher probability than if the study was less probable to begin with.
...
If we equate riskiness with a low probability of obtaining evidence, the Bayesian rationale perfectly aligns with the observation that risky predictions lead to persuasive evidence. <!-- I believe this is the wrong conceptualization. Riskiness refers to the probability of obtaining the evidence given the Hypothesis is false, not to the total probability of the evidence. I would swap Eq. 1 for a version that already entails Eq. 2 -->
This tension between high risk leading to high reward is central to our consideration of preregistration.
A high-risk, high-reward strategy is bound to result in many losses that are eventually absorbed by the high gains. <!-- I think this is not true. A high-risk strategy means conducting studies where it is unlikely to observe the evidence, given the hypothesis is wrong. However, if the hypothesis is true, and P(E|H) = 1, there are no losses, even in risky studies. -->
...
We already have noted that there is not much to do about prior probability ($P(H)$ and that it is common sense to increase detectability ($P(E|H)$). <!-- the question is not whether it is common sense, but whether it has something to do with preregistration -->
...
Its counter probability $P(¬E|¬H)= 1 - P(E|¬H)$ is what we call "theoretical risk", because it is the risk a theory takes on in predicting the occurrence of particular evidence in its favour. <!-- I am not shure if thats why we call it theoretical risk. I am also not shure if this is gramatically sensible. -->
...
Second, if the theoretical risk is smaller than the detectability ($P(E|H)$) it follows that the posterior probability must decrease when observing the evidence.
...
In other words, observing a fact that would also be observed if the hypotheses was wrong does not lend credence to the hypothesis.
...
Both theoretical risk ($P(¬E|¬H)$) and detectability ($P(E|H)$) aggregate uncountable influences, otherwise they could not model the process of evidential support for theories. <!-- I dont understand this sentence -->
...
These examples were created to illustrate how experiments may differ in their theoretical risk and detectability.
...
In the unlikely event that all 10 students place their matches to form the Roman numeral Ⅷ ($E_3$), it is probably due to insight ($H$) and not by chance $P(¬E_2|¬H)$).
...
<!-- I think this example needs to be swapped for a simpler one. For example: H: A coin is fair. E1: Observe if the person throwing the coin is betwenn 0 and 10 meters in height (high detectability, no theoretical risk). E2: Throw the coin 2 times and observe if it lands one time on heads and one time on tails (detectability: P(E|H) = 0.5, theoretical risk: P(E|p != 0.5) = 1/3) E3: throw the coin 200 times, compute #heads/#200, construct a test (high power -> high detectability, high theoretical risk). -->
As argued earlier, the remainder of the paper will focus on binary, non-probabilisitic evidence to keep the mathematical notation as simple as possible. <!-- what is non-probabilistic evidence supposed to be? Evidence is always a random variable -->
...
Indeed, limiting the type-I error rate is commonly stated as a goal of preregistration.
...
The point, however, is that we want to show that preregistration increases the expected epistemic value regardless of the theory being tested. <!-- I think the example is too theoretical and i dont understand the conclusions drawn from it -->
...
However, the research hypothesis seldomly equals  the statistical null hypothesis, and therefore, the relation between statistical type-I error rate and theoretical risk should not be overinterpreted. <!-- what is it now, the null or the alternative? -->
We argue that theoretical risk also encompasses factors outside the statistical realm, most notably the study design and broader analytical strategies.
...
The only thing we can conclude with some certainty is that theoretical risk is not higher than what the statistical model guarantees without knowledge about the other factors at play. <!-- why are there no factors that increase theoretical risk? -->
...
Enforcing this distinction would mean using preregistrations as a means to increase the theoretical risk researchers take on.
...
In our view, the advantage of uncertainty reduction about theoretical risk front and center is two-fold.
First, aiming to reduce uncertainty is beneficial across a wide range of potential theoretical risks.
That is, researchers benefit from preregistering their study, regardless of whether or not the study is "exploratory".
...
Second, if researchers realize after the fact that it would be unwise to follow their preregistration in certain details, they may change their strategy.
Allowing researchers to deviate from the preregistration is controversial (XXX) because the authors might sift through all the possibilities and then select the most beneficial for their theory.
In its fullest consequence, this argument might result in the same uncertainty that we argued plagues for not-preregistered studies.
However, if the authors offer a convincing argument for their deviation, their readers might may judge it to be a reasonable explanation for the deviation.
In fact, using the here presented framework, we can distinguish reasonable from unreasonable deviations.
If a change increases detectability enough to offset the increase in uncertainty in theoretical risk, it is worthwhile.
