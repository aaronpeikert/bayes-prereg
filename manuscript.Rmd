...
A successful prediction should lend more credibility to a theory than a "postdiction," everything else being equal.
...
If the research community remains confused about the purpose of preregistration, it is bound to misapply it and hence decrease the efficiency of the scientific endeavor. 
...
Insisting on equating preregistration and confirmation has, hence, led to criticism that preregistration is actually harmful all things considered, and neither sufficient nor necessary to establish confirmation [@szollosiPreregistrationWorthwhile2020; @phamPreregistrationNeitherSufficient2021]
...
Equating confirmatory research with preregistered research is only possible for studies that expect a low type-I error rate and will run/be analyzed without changes no matter whatin any case [@simmonsPreregistrationWhyHow2021; @bakkerEnsuringQualitySpecificity2020].
...
To make our three arguments, we must assume three things about this estimation process and how it relates to what studies to conduct.
...
We restrict ourselves to evidence of a binary nature (either it exists or does not) since continuous evidence would lead to some quite involved derivations.
...
For the purpose of illustration, we defin it, with quite some hand-waving, as a cognitive abstraction that allows agents to consistently solve a well-defined class of problems.
...
We chose this uniform distribution to capture our statement that a statistical model only guarantees an upper bound to theoretical risk (and since it is a probability the lower bound is 0) as it is the maximum entropy distribution under this assumption and thus conforms to our Bayesian framework [@giffinUpdatingProbabilitiesData2007].
...
Communicating clearly what authors did to arrive at the evidence they present is crucial for judging the theoretical risk they took.
...
If the researchers have described what they intended to do beforehand, and then report that they did exactly that, the readers can be certain, that they have received a complete account of the situation.
...
Let us note that a study that does not result in the evidence a researcher is hoping for is still valuable for the scientific process.
...
That is, researchers benefit from preregistering their study, regardless of whether the study is "exploratory" or not.
