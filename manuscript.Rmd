...
One reason may be that some do not find that the theoretical advantages outweigh the practical hurdles, such as specifying every aspect of a theory in advance. <!-- it thought more of an analysis than a theory -->
...
Researchers may apply two strategies to evade the self-imposed restrictions of preregistrations: writing a loose preregistration to begin with [@stefanBigLittleLies2022] or deviating from the preregistration afterwards. <!-- harsh transition -->
...
We therefore attempt to disentangle the *methodological* goals of preregistration from the *ideological* <!-- the word ideological may have a negative connotation. --> goals of confirmatory science.
It is likely that psychology needs more confirmatory studies to progress as a science, but quite independently of this, preregistration can be useful for any kind of study on the continuum between strictly confirmatory and fully exploratory. <!-- I think psychology does not need more confirmatory studies, and also this is not really the focus of this paper, so I would remove this. I believe psychology needs more confirmatory studies that are really confirmatory (and not p-hacked) and more exploratory studies that don't disguise as confirmatory. Using preregistration in a way that quantifies where a study is on this continuum would maybe help to encourage people making more (semi-)exploratory studies, I believe this is a point that you could make here, if you want to. -->
...
Further, we outline two interpretations of preregistration. The first one, commonly found among applied scientists, is that preregistration impacts theoretical risk. <!-- I think your second interpretation is all about the fact that preregistration does not (mainly) impact theoretical risk, but the uncertainty about it. -->
It corresponds to the traditional application of preregistration to research paradigms that focus on confirmation by maximizing the theoretical risk or equivalently by limiting type-I error (when dichotomous decisions about theories are an inferential goal).
We argue that this view on the utility of preregistration can be interpreted as maximization of theoretical risk, which is reduced by researchers' degrees of freedom, p-hacking, and suchlike.
The second interpretation is our main contribution: We argue that, in contrary to the classic view, the objective of preregistration is *not* the maximization of theoretical risk, but rather the minimization of uncertainty about the theoretical risk. This interpretation leads to a broad applicability of preregistration to both exploratory and confirmatory studies.
...
We conclude that because preregistration decreases uncertainty about the theoretical risk, which in turn increases the amount of confirmation we gain from a particular study<!-- I would rephrase this... atm it sounds a bit like an increase in P(E|H), but you mean an increase in P(H|E) -->, preregistration is potentially useful for any kind of study, no matter how exploratory.
...
In the tradition of Carnap, in its direct use, it is called confirmation as firmness; in its relation to the a priori probability $P(H)$, it is called increase in firmness [@carnapLogicalFoundationsProbability1950, preface to the 1962 edition].
...
We referred to this assumption as "statistical relevance condition" earlier [@wesleyc.salmonStatisticalExplanation1970; @fetzerStatisticalExplanations1974]. <!-- I believe it is confirmation, not corroboration. Also: do we really need the statistical relevance condition? What is the relation between the relevance condition and the monotinic increase in confirmation? Also I think you should cite literature that says that this monotonic increase is indeed a reasonable assumption. -->

In short, we want to increase posterior probability $P(H|E)$.
Increases in posterior probability $P(H|E)$ are associated with increased epistemic value. <!-- what is the difference between epistemic value and confirmation? -->
...
We call this probability of detecting evidence given that the theory holds "detectability." <!-- citation for the source of "detectability" -->
...
In other words, high risk, high reward. <!--I believe this is not true. P(E) is the sum of two terms (as you write below). Increasing the first term will increase posterior probability, increasing the second term will decrease posterior probability.
High risk, high reward does not refer to P(E), it refers to P(H) and P(\not E| \not H): assume the researcher is wrong with his hypothesis, how probable is it that the researcher will actually be proven wrong. As P(E|\not H) = 1- P(\not E| \not H), increasing the riskiness while P(H) is low will decrease the second term in the denominator, thereby increasing P(H|E) -->

If we equate riskiness with a low probability of obtaining evidence under the assumption that our theory is wrong, the Bayesian rationale perfectly aligns with the observation that risky predictions lead to persuasive evidence. <!-- se comment before -->
This tension between high risk leading to high gain is central to our consideration of preregistration.
A high-risk, high-gain strategy is bound to result in many losses that are eventually absorbed by the high gains. <!-- Keep in mind this is only true for Hypotheses with low P(H). If P(H) is high, it does not really matter how risky your study is, if detectability is high, you will confirm the hypothesis (e.g. this corresponds to a setting where you test the certain consequence of a certain hypothesis, H: no living human is larger than 500 meters E: measure how tall every human on earth is. P(E|H) = 1, P(\not E| \not H) = 1, so P(H|E) = P(H) -->
...
Using the law of total probability, we can split $P(E)$ into two terms: <!-- I would move this further above to beeing able to better explain riskiness -->
...
Third, if the theoretical risk equals zero, then posterior probability is at best equal to prior probability but only if detectability is perfect ($P(H|E)$ = 1). <!-- sure? -->
...
1. The experimenter gives a hint that  the problem is easy to solve when using Roman numerals; if all students come up with the solution, she records it as evidence for the hypothesis. <!-- I dont really yunderstand how this relates to insight... -->
...
This is not a risky prediction to make in psychology [@orbenCrudReDefined2020]. <!-- I dont know if the citation relates to alpha-inflation or to the riskiness of such predictions -->
...
<!-- This is true but I think it does not really relate to preregistration. With the same amount of resources, preregistration can limit researcher degrees of freedom, thereby increasing theoretical risk. -->

# Uncertainty about theoretical risk 

We have established that higher theoretical risk leads to more persuasive evidence.
In other words, we have reconstructed the interpretation that preregistrations supposedly works by restricting the researchers, which in turn increases the theoretical risk (or equivalently limits the type-I error rate) and thereby creates more compelling evidence.
...
To that end, we incorporate uncertainty about theoretical risk into our framework. <!-- you already have a lot of uncertainty in your framework, you now also consider this one -->

## Statistical methods

One widely known factor is the contribution of statistical methods to theoretical risk.
Theoretical risk $P(\neg{}E|\neg{}H)$ is deeply connected with statistical methods, because it is related to the type-I error rate in statistical hypothesis testing $P(E|\neg{}H)$ by $P(\neg{}E|\neg{}H) = 1 - P(E|\neg{}H)$<!-- the inverse would be ^-1-->, if you consider the overly simplistic case where the research hypothesis is equal to the statistical alternative hypothesis, because then the nill hypothesis is $\neg{}H$. <!-- I believe this would only be true, if the occurence of a non-significant result would lead to a refutation of the theory. In NHST, theoretically P(\not E | \not H) may be the theoretical risk (the probability ob observing no effect under the assumption that the N-Hypothesis is true), but in practice, researchers don't count this as evidence against the alternative hypothesis - therefore, if you adapt the framework of NHST, theoretical risk is always zero and no confirmation at all takes place. -->
...
However, the research hypothesis seldomly equals  the statistical null hypothesis, and therefore, the relation between statistical type-I error rate and theoretical risk should not be overinterpreted. <!-- above you state this the other way around: research hypothesis = alternative hypothesis -->
We argue that theoretical risk (and hence its inverse, $P(E|\neg{}H)$) also encompasses factors outside the statistical realm, most notably the study design and broader analytical strategies. <!-- yes!:) -->
...
These tools help us account for biases in statistical methods that influence theoretical risk.
...
The precision with which statisticians can discuss contributions to theoretical risk has lured the community concerned with research methods <!-- I think this is to broad... in other sciences, the modeling part may be more important than the statistical part. -->into ignoring other factors that are much more uncertain.
...
Let's assume a researcher is reading a study from another lab and tries to decide whether and how much the presented results confirm the theory. As the researcher did not conduct the study (and the study is not preregistered), they can not be certain about the various factors influencing theoretical risk (researcher degrees of freedom).
We therefore express this uncertainty about the theoretical risk as a probability distribution $Q$ of $P(E|\neg{}H)$ (remember that $P(E|\neg{}H)$ is related to theoretical risk by $P(E|\neg{}H)$ = 1 -$P(\neg{}E|\neg{}H)$, so it does not matter wheter we consider the distribution of theoretical risk or $P(E|\neg{}H)$).
...
Of course, the assigned probabilities and the distribution $Q$ varies from study to study and researcher to researcher, but we can illustrate the effect of uncertainty with a sensible example.
Assuming a power of 80\% we assign $P(E|H) = 0.8$, we assume the tested hypotheses is considered unlikely to be true by the research community before the study is conducted ($P(H) = 0.1$) and assign a uniform distribution for $P(E|\neg{}H) \sim U([\tau, 1])$ where $\tau{}$ is set to the nominal type-I error rate of the statistical model, reflecting our assumption that this error rate gives a lower bound for $P(E|\neg{}H)$.
We chose this uniform distribution as it is the maximum entropy distribution with support $[\tau, 1]$ and hence conforms to our Bayesian framework [@giffinUpdatingProbabilitiesData2007].

With this, we derive the expected value of $P(H|E)$ as
\begin{align}
\mathbb{E}_Q[P(H|E)] &= \mathbb{E}_Q \left[ \frac{P(H)P(E|H)}{P(H)P(E|H)+P(\neg H) P(E|\neg H) } \right] \\[1em]
&= \int_\mathbb{R} \frac{P(H)P(E|H)}{P(H)P(E|H)+P(\neg H) P(E|\neg H) } \, \mathrm{d} Q \\[1em]
&= \int_{[\tau, 1]} (1-\tau)^{-1} \frac{P(H)P(E|H)}{P(H)P(E|H)+P(\neg H) x } \, \mathrm{d} x \\[1em]
&= \frac{P(H)P(E|H)}{P(\neg{}H)(1-\tau)}\ln \left(\frac{P(H)P(E|H) + P(\neg{}H)}{P(H)P(E|H) + P(\neg{}H)\tau}\right)\label{eq:expectation}
\end{align}
...
<!-- I think the deleted sentence is not true - with "fully uncertain" you mean you use the maximum entropy distribution with support [\tau, 1], but I could for example realistically assume that in most cases, P(E|\not H) is larger than the nominal type 1 error rate, leading to a distribution that places more mass towards a higher value of P(E|\not H), leading to a lower expected value for P(H|E) -->
...
<!-- same as before: take a low-entropy distribution that places its weight on high values of P(E|\not H) -> this leads to a lower expected value than the high entropy distributions. -->
...
The other factors (detectability, prior beliefs, measure of epistemic value) and, therefore, the extend of the benefit vary of course with the specifics of the study.
Crucially, even studies with some exploratory aspects benefit from preregistration, e.g., in this scenario with a $\tau$ = `r some_exploration_tau`, so false positive rate of `r 1-some_exploration_tau`, moving from uncertain to certain increases the posterior by a factor of `r round(some_exploration_certain/some_exploration_uncertain, 2)` <!-- I would prefer reporting the prior probability and the posteriors in dependence of certainty/uncertainty over report the ratio between the posteriors-->.
...
From the perspective of the ultimate sceptic, they may claim maximum uncertainty <!--as before: I think the ultimate sceptic would place more weight on the higher end of the distribution, resulting in a less entropic distribution -->.
...
In that case, readers can be certain they received a complete account of the situation. They still might be uncertain about the actual theoretical risk the authors took, but to a much smaller extent than if the study would not have been preregistered.
...
Therefore, a study that perfectly adhered to its preregistration will resemble the solid line in Figure \ref{fig:posterior}/\ref{fig:other-measures}. <!-- I wonder if it would be beneficial to include a low-uncertainty distribution in the figure. Because there is not a linear mapping between "how uncertain" -> "expected value", so we might want to make shure that we don't arrive at a low expected value at the slightest uncertainty -->
...
Specifying the process, therefore, is in line with the rational we propose here, but is less reasonable when to goal of preregistration is supposed to be a strictly confirmatory research agenda.
...
<!-- I think this paragraph is a bit long and repetitive -->
...
However, while a PAC is not the extend of exploration, it is very specific about exactly how probable it is to generate evidence even when the theory does not hold (theoretical risk). <!-- I don't understand "while a PAC is not the extend of exploration" -->
...
Clearly communicating theoretical risk is important because it reduces uncertainty and hence increases expected epistemic value.
...
The manuscript is available as [preprint](https://www.doi.org/10.31234/osf.io/cs8wb) [@peikertWhyDoesPreregistration2023] and was submitted to [Philosophical Psychology](https://www.tandfonline.com/journals/cphp20).
